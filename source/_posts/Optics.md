...
其中 L 是总采样长度（物理世界的实际范围），N 是采样点数，Δu 是采样间距。
...
**物理意义**：FFT将 `[0, 1/Δu]` 的频率范围分成 `N` 份，每份间隔为 `Δf`，
FFT输出矩阵中，索引为 (i, j) 的点对应的复数值，是光瞳函数中空间频率为 ((i-N//2)·Δf, (j-N//2)·Δf) 的复振幅分量。
...
> **示例**  
> $Δx_f$ =` (632.8e-9 × 3.5e-3) / (51 × 100e-6) = 4.34×10⁻⁷ m/pixel = 0.434 µm/pixel`
